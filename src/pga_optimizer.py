...
class PGA_Optimizer:
            ...
            for row in reader:
                ...
                if player_name in self.player_dict:
                    ...
                    self.player_dict[player_name]['Ceiling'] = float(
                            row['ceiling'])
                    self.player_dict[player_name]['Floor'] = float(
                            row['floor'])
                    self.player_dict[player_name]['Top6%'] = float(
                            row['top6%'])
                    self.player_dict[player_name]['Withdraw'] = float(
                            row['Withdraw'])
                    self.player_dict[player_name]['Win%'] = float(
                            row['win%'])
                    self.player_dict[player_name]['Swt6%'] = float(
                            row['swt6%'])
        ...
        with open(path, encoding='utf-8-sig') as file:
            ...
            for row in reader:
                ...
                if float(row['Total Pts']) < self.projection_minimum:
                    continue
                self.player_dict[player_name] = {'Fpts': 0.1, 'ID': 0, 'Salary': 1000, 'Name': '', 'RealID': 0, 'Ownership': 0.1, 
                                                 'Optimal': 0.1, 'Leverage': 0.0, 'StdDev': 0.1, 'Ceiling': 0.0, 'Floor': 0.0, 'Top6%': 0, 'Withdraw': 0,
                                                 'Win%': 0, 'Swt6%': 0}
    ...
    def optimize(self):
        ...
        if 'min_lineup_salary' in self.config and self.config['min_lineup_salary'] is not None:
        ...
        if self.site == 'dk':
            ...
            self.problem += plp.lpSum(lp_variables[player]
                                      for player in self.player_dict) >= 6
            self.problem += plp.lpSum(lp_variables[player]
                                      for player in self.player_dict) <= 6
        ...
        with open(out_path, 'w') as f:
            if self.site == 'dk':
                ...
                for fpts, x in self.lineups.items():
                    ...
                    top6_p = np.prod([self.player_dict[player]['Top6%'] / 100.0 for player in x])
                    win_p = np.prod([self.player_dict[player]['Win%'] / 100.0 for player in x])
                    leverage_s = sum([self.player_dict[player]['Leverage'] for player in x])
                    swt6_p = np.prod([self.player_dict[player]['Swt6%'] / 100.0 for player in x])
                    withdraw_p = np.prod([self.player_dict[player]['Withdraw'] / 100.0 for player in x])
                    ...
                    lineup_str = '{} ({}),{} ({}),{} ({}),{} ({}),{} ({}),{} ({}),{},{},{},{},{},{},{},{},{},{},{}'.format(
                        ...
                        salary, round(
                            fpts_p, 2), ceil, own_p, own_s, optimal_p, top6_p, win_p, leverage_s, swt6_p, withdraw_p
                    )
                    f.write('%s\n' % lineup_str)
            else:
               ## TODO  - add fd
