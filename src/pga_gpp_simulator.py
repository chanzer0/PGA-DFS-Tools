        ...
        with open(path) as file:
            if self.site == 'dk':
                ...
                for i,row in reader.iterrows():
                    ...
                    if i == self.field_size:
                        break
                    lineup = [row[0].split('(')[0][:-1].replace('-', '#').lower(), row[1].split('(')[0][:-1].replace('-', '#').lower(),
                              row[2].split(
                        '(')[0][:-1].replace('-', '#').lower(), row[3].split('(')[0][:-1].replace('-', '#').lower(),
                        row[4].split(
                        '(')[0][:-1].replace('-', '#').lower(), row[5].split('(')[0][:-1].replace('-', '#').lower()]
